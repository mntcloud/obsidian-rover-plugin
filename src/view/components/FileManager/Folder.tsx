  ...
  view(vnode: m.Vnode<Attr, this>) {
    return (
      <div
        ...
        oncreatedelete={async (ev: CustomEvent<{ complete: () => void }>) =>
          await this.onCreateDelete(vnode.attrs.path, ev.detail.complete)
        }
      ...
      >
        <div
          className={`rover-folder ${this.nested.length ? "expanded" : ""}`}
          draggable={true}
        ...
        >
          {this.nested.length ? (
            <svg
              ...
              class="rover-lucide lucide-folder lucide-folder-open"
            >
