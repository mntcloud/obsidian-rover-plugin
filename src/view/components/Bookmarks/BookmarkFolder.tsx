...
export class BookmarkFolder implements m.ClassComponent<Attr> {
  ...
  view(vnode: m.Vnode<Attr, this>) {
    return (
      <div className="rover-bookmark-folder-container" data-crd={vnode.key}>
        {Bookmarks.dragged &&
        ((Bookmarks.dragged.pos.length > vnode.attrs.position.length &&
          Bookmarks.dragged.pos[vnode.attrs.position.length - 1] >=
            vnode.attrs.position[vnode.attrs.position.length - 1]) ||
          (Bookmarks.dragged.pos.length <= vnode.attrs.position.length &&
            Bookmarks.dragged.pos[Bookmarks.dragged.pos.length - 1] >
              vnode.attrs.position[Bookmarks.dragged.pos.length - 1])) ? (
          <Space
            crd={vnode.key}
