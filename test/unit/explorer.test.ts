import { describe, test, before, mock, MockModuleContext } from "node:test";
...
describe("Vault Listeners", () => {
  ...
  test("on folder rename in root", async () => {
    ...
    renameMap.set("folderB/subfolder1/file2.md", "folderA/subfolder1/file2.md");
    ...
    for (const [newPath, oldPath] of renameMap.entries()) {
      await Explorer.onRename(newPath, oldPath, onUpdate);
    }
  ...
  test("move folder and rename it at the same time", async () => {
    ...
    assert.strictEqual(onUpdate.mock.calls.length, 2);
  });
  ...
  test("move folder from root", async () => {
    ...
    const onUpdate = mock.fn(async (isRoot: boolean, path?: string) => {
      if (isRoot) {
        ...
        assert.match(path!, /folderB\/folderC/);
      }
    });
  ...
  test("move file from root to different location", async () => {
    ...
    mockedObsidian?.app.vault.adapter.stat.mock.mockImplementation(
      async (path: string) => {
        ...
        if (path === "folderB" || path === "folderB/folderC" || path === "") {
          return {
            type: "folder",
            ctime: Date.now(),
            mtime: Date.now(),
  ...
  test("move folder with a little of repeating paths", async () => {
    ...
    const onUpdate = mock.fn(async (isRoot: boolean, path?: string) => {
      ...
      assert.ok(["folderB", "folderD/folderB"].includes(path!));
    });
  ...
  test("move folder with children to deeply nested location", async () => {
    ...
    const onUpdate = mock.fn(async (isRoot: boolean, path?: string) => {
      if (isRoot) {
        ...
        // Expect updates for the old parent and the new parent path
        assert.match(path!, /deeply\/nested\/destination|parentFolder/);
      }
    });
